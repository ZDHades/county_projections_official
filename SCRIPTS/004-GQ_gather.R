...
cat("[LOG] Starting 004-GQ_gather.R...\n")
...
getgq_2010 = function(x) {
  tryCatch({
    cat("[LOG] [START] getgq_2010 for state:", x, "\n")
    ...
    cat("[LOG] Reading P012 (total pop) as CSV...\n")
    p12_file <- "DATA/2010_SF1_TX/tx2010.sf1/tx000042010.sf1"
    p12 <- readr::read_csv(p12_file, col_names = FALSE, show_col_types = FALSE)
    cat("[LOG] p12 dim:", dim(p12), "\n")
    print(head(p12, 2))
    cat("[LOG] Finished reading P012. Rows:", nrow(p12), "\n")
    cat("[LOG] Reading PCT013 (household pop) as CSV...\n")
    pct13_file <- "DATA/2010_SF1_TX/tx2010.sf1/tx000182010.sf1"
    pct13 <- readr::read_csv(pct13_file, col_names = FALSE, show_col_types = FALSE)
    cat("[LOG] pct13 dim:", dim(pct13), "\n")
    print(head(pct13, 2))
    cat("[LOG] Finished reading PCT013. Rows:", nrow(pct13), "\n")
    # Assign LOGRECNO from X6 (the 6th column)
    p12$LOGRECNO <- as.character(p12$X6)
    cat("[LOG] p12 LOGRECNO sample:", head(p12$LOGRECNO, 2), "\n")
    pct13$LOGRECNO <- as.character(pct13$X6)
    cat("[LOG] pct13 LOGRECNO sample:", head(pct13$LOGRECNO, 2), "\n")
    cat("[LOG] Merging geo and P012...\n")
    df_merged <- merge(geo, p12, by="LOGRECNO")
    cat("[LOG] df_merged dim:", dim(df_merged), "\n")
    print(head(df_merged, 2))
    cat("[LOG] df_merged column names after merge: ", paste(colnames(df_merged), collapse=", "), "\n")
    cat("[LOG] First row after merge:\n")
    print(head(df_merged, 1))
    # Map V# columns to P012 variables based on SF1 layout
    # V6 = P0120001 (Total), V7 = P0120002 (Male), V30 = P0120026 (Female)
    cat("[LOG] Converting X7 (Total), X8 (Male), X32 (Female) to numeric...\n")
    df_merged$TOTAL_POP <- as.numeric(df_merged$X7)
    df_merged$MALE <- as.numeric(df_merged$X8)
    df_merged$FEMALE <- as.numeric(df_merged$X32)
    cat("[LOG] Example values after conversion:\n")
    print(head(df_merged[, c('TOTAL_POP', 'MALE', 'FEMALE')]))
    # Find columns from P012 (those starting with 'X' and are numeric data columns)
    p12_cols <- grep('^X[0-9]+$', colnames(df_merged), value=TRUE)
    cat("[LOG] Found P012 columns:", length(p12_cols), "columns\n")
    cat("[LOG] First few P012 columns:", paste(head(p12_cols, 5), collapse=", "), "\n")
    
    # Convert these columns to numeric and sum them
    p12_data <- df_merged[, p12_cols]
    ...
    print(head(df2, 2))
    cat("[LOG] Summing PCT013 pop columns...\n")
    # Find columns that end with _pct13 (these are from the PCT013 file)
    pct13_cols <- grep('_pct13$', colnames(df2), value=TRUE)
    cat("[LOG] Found PCT013 columns:", length(pct13_cols), "columns\n")
    cat("[LOG] First few PCT013 columns:", paste(head(pct13_cols, 5), collapse=", "), "\n")
    
    # Convert these columns to numeric and sum them
    pct13_data <- df2[, pct13_cols]
    pct13_data <- sapply(pct13_data, as.numeric)
    df2$HHPOP <- rowSums(pct13_data, na.rm=TRUE)
    ...
    print(head(out, 2))
    cat("[LOG] [END] getgq_2010. Output rows:", nrow(out), "\n")
    return(out)
